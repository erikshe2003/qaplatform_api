...
# 5.将token写入redis
# 6.返回信息
@user_apis.route('/login.json', methods=["post"])
def login_json():
    # 初始化返回内容
    response_json = {
        "error_code": 200,
    ...
    response_json["data"]["user_info"]["access_token"] = user_token
    response_json["data"]["user_info"]["nick_name"] = userdata["userNickName"]
    response_json["data"]["user_info"]["mail_address"] = userdata["userEmail"]
    response_json["data"]["user_info"]["status"] = userdata["userStatus"]

    # 4.读取账户权限信息，封装基础信息
    # 如果账户未分配角色（新账户），则permission留空
    if userdata["userRoleId"] is None or userdata["userRoleId"] == "" or type(userdata["userRoleId"]) is not int:
        response_json["data"]["permission"] = {}
    # 否则
    else:
        roledata = ApiCheck.check_role_permission(userdata["userRoleId"])
        if roledata["exist"] is False:
            return ApiError.requestfail_error("账户所属角色信息异常")
        elif roledata["exist"] is True:
            pass
        else:
            return ApiError.requestfail_error("账户所属角色信息校验异常")
        response_json["data"]["permission"] = roledata["rolePermission"]
