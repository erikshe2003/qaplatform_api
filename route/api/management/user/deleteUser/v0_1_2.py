...
@api_management_user.route('/deleteUser.json', methods=["post"])
def delete_user():
    # 初始化返回内容
    response_json = {
        "error_code": 200,
    ...
    # status
    # 仅已删除/已禁用/正常状态的账户支持删除操作
    if type(request_json["status"]) is not int or request_json["status"] not in [-2, -1, 1]:
    ...
    try:
        ...
        if ur_data and ur_data.roleIsAdmin in [0, None]:
            ...
            return ApiError.requestfail_error("管理员账号禁止删除")
        else:
