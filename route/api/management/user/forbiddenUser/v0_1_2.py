...
@api_management_user.route('/forbiddenUser.json', methods=["post"])
def forbidden_user():
    # 初始化返回内容
    response_json = {
        "error_code": 200,
    ...
    try:
        ...
        logmsg = "/forbiddenUser.json数据格式化失败，失败原因：" + repr(e)
    ...
    # status
    if type(request_json["status"]) is not int or request_json["status"] not in [-1, 1]:
    ...
    try:
        ...
        if ur_data and ur_data.roleIsAdmin in [0, None]:
            ...
            return ApiError.requestfail_error("管理员账号禁止禁用")
        elif ur_data and ur_data.userStatus in [-2, 0]:
            return ApiError.requestfail_error("已删除或未激活的账号禁止禁用")
        else:
